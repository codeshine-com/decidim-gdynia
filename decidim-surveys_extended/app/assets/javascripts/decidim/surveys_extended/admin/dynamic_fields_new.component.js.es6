((exports) => {
  class NewDynamicFieldsComponent {
    constructor(options = {}) {
      ...
      // metrics
      this.addMetricsButtonSelector = options.addMetricsButtonSelector;
      this.metricsTemplateSelector = options.metricsTemplateSelector;
    ...
    _bindEvents() {
      ...
      if (this.addMetricsButtonSelector) {
        $(this.wrapperSelector).on("click", this.addMetricsButtonSelector, (event) =>
          this._bindSafeEvent(event, () => this._addMetricFields())
        );
      }
    ...
    _addField(templateClass = ".decidim-template", metrics = null) {
      ...
      if (['gender', 'age', 'district'].includes(metrics)) {
        if ((metrics == 'gender') &&  $('#gender').is(':checked')) {
          if ($('#gender_required').is(':checked')) {
            $newField.find(".metrics-required").prop('checked', true);
          }
          $newField.find(".metrics-body").val('Wybierz płeć');
          $newField.find(".metrics-type").val(metrics);
          $newField.find(".question-title-statement").text('Pytanie o płeć');
          // remove
          $('.column-metrics-gender').remove();
        } else if ((metrics == 'age') && $('#age').is(':checked')) {
          if ($('#age_required').is(':checked')) {
            $newField.find(".metrics-required").prop('checked', true);
          }
          $newField.find(".question-title-statement").text('Pytanie o wiek');
          $newField.find(".metrics-type").val(metrics);
          $newField.find(".metrics-body").val('Wybierz wiek');
          // remove
          $('.column-metrics-age').remove();
        } else if ((metrics == 'district') && $('#district').is(':checked')) {
          if ($('#district_required').is(':checked')) {
            $newField.find(".metrics-required").prop('checked', true);
          }
    ...
    _addMetricFields() {
      ...
      this._addField(this.metricsTemplateSelector, 'gender'),
      this._addField(this.metricsTemplateSelector, 'age'),
      this._addField(this.metricsTemplateSelector, 'district')
    }
  ...
  exports.DecidimAdmin.NewDynamicFieldsComponent = NewDynamicFieldsComponent;
  exports.DecidimAdmin.createDynamicFieldsNew = (options) => {
    return new NewDynamicFieldsComponent(options);
  };
})(window);
